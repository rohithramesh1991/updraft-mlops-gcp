...
def classification_pipeline(project: str, dataset: str, table: str):
    ...
    # CustomTrainingJobOp for training with explicit machine type
    ...
    t = train_job(x_train_path=p.outputs["x_train_path"], y_train_path=p.outputs["y_train_path"])

    e = evaluate_op(model_path=t.outputs["model_path"],
                    x_test_path=p.outputs["x_test_path"],
                    y_test_path=p.outputs["y_test_path"])

    # Upload and deploy the trained model
